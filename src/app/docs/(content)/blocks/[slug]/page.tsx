...
const CONTENT_DIR = path.join(process.cwd(), "content", "blocks");
...
export async function generateMetadata({ params }: { params: { slug: string } }) {
   ...
   for (const candidate of candidates) {
      ...
      try {
         const mod = (await import(`@/content/blocks/${candidate}.mdx`)) as {
            metadata?: {
               title?: string;
               description?: string;
            };
         ...
         return {
            ...
            alternates: {
               canonical: `/docs/(content)/blocks/${slug}`,
            },
         };
      } catch {}
   }
...
export default async function DocsBlocksSlugPage({ params }: { params: { slug: string } }) {
   const { slug } = await params;
   const candidates = Array.from(new Set([slug, slug.replace(/-/g, "")]));
   ...
   for (const candidate of candidates) {
      ...
      if (!fs.existsSync(candidatePath)) continue;
      try {
         Mod = await import(`@/content/blocks/${candidate}.mdx`);
         break;
      } catch {}
   }
