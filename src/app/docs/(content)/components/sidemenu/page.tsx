...
export default function Sidemenu1Page() {
    const sidemenu1Count = Object.keys(sidemenu1Data).length;
    
    // Separate cards into odd and even indices
    const oddIndexCards = [];
    const evenIndexCards = [];
    
    for (let i = 0; i < sidemenu1Count; i++) {
        if (i % 2 === 0) {
            ...
            <div className="grid grid-cols-2 gap-4 group">
                ...
                <div className="flex flex-col gap-4">
                    {oddIndexCards.map((index) => (
                        <Link key={index} href={`/docs/components/sidemenu/${index + 1}`} className="flex flex-col gap-4 group/card">
                            <Image
                                src={`/components/sidemenu/sidemenu${index + 1}.png`}
                                alt={`Sidemenu ${index + 1}`}
                                width={500}
                                height={500}
