...
import messageThreadData from "@/app/docs/(content)/data/messageThread.json";

export default function MessageThreadPage() {
    const messageThreadCount = Object.keys(messageThreadData).length;
            ...
            <div className="grid grid-cols-2 gap-4 group">
                ...
                <div className="flex flex-col gap-4">
                    {oddIndexCards.map((index) => (
                        <Link key={index} href={`/docs/components/messageThread/${index + 1}`} className="flex flex-col gap-4 group/card">
                            <Image
                                src={`/components/messageThread/messageThread${index + 1}.png`}
                                alt={`Message Thread ${index + 1}`}
                                width={500}
                                height={500}
