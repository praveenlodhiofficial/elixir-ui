...
export async function generateMetadata({ params }: { params: { slug: string } }) {
   ...
   for (const candidate of candidates) {
      ...
      try {
         const mod = (await import(`@/content/templates/${candidate}.mdx`)) as {
            metadata?: {
               title?: string;
               description?: string;
            };
         ...
         return {
            ...
            alternates: {
               canonical: `/docs/(content)/templates/${slug}`,
            },
         };
      } catch {}
   }
...
export default async function DocsTemplatesSlugPage({ params }: { params: { slug: string } }) {
   const { slug } = await params;
   const candidates = Array.from(new Set([slug, slug.replace(/-/g, "")]));
   ...
   for (const candidate of candidates) {
      ...
      if (!fs.existsSync(candidatePath)) continue;
      try {
         Mod = await import(`@/content/templates/${candidate}.mdx`);
         break;
      } catch {}
   }
