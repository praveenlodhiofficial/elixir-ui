...
export default function Teammates({ 
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      setOpenedIndex((prev) => (prev + 1) % teammates.length);
    }, 1500);
    return () => clearTimeout(timer);
  }, [openedIndex, teammates.length, hoveredTeammate]);

  // Determine current teammate for image (sync with openedIndex if not hovered)
  const currentTeammate = hoveredTeammate
    ? teammates.find(t => t.id === hoveredTeammate)
    : (openedIndex >= 0 ? teammates[openedIndex] : teammates[0]);
              ...
              <motion.div
                ...
                onMouseLeave={() => {
                  ...
                  if (lastHoveredIndex !== null) {
                    setOpenedIndex((lastHoveredIndex + 1) % teammates.length);
