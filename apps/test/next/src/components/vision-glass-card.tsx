...
export function VisionGlassCard({
  ...
  /* FIXED (no nested springs) */
  const specXRaw = useTransform(x, [-100, 100], [50, 200]);
  const specYRaw = useTransform(y, [-100, 100], [50, 200]);

  const specX = useSpring(specXRaw, SPRING);
  const specY = useSpring(specYRaw, SPRING);
  ...
  return (
    <div
      ...
      onMouseLeave={() => {
        rawX.set(0);
        rawY.set(0);
        scaleRaw.set(1.3); // FIXED consistency
      }}
      style={{
        width: 256,
        aspectRatio: "11/16",
